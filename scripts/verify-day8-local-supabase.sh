#!/usr/bin/env bash
# Verify Day 8 accountant-center migrations and RLS on the disposable local Supabase stack.
# Resets the isolated local database unless DAY8_DB_RESET=0. Does not touch production.
...
echo "Day 8 local Supabase verification"
...
if [[ "${DAY8_DB_RESET:-1}" == "1" ]]; then
  ...
  if ! npx --yes supabase db push --local >/tmp/sts-day8-db-push.out 2>&1; then
    if ! npx --yes supabase migration up --local >/tmp/sts-day8-db-push.out 2>&1; then
      python3 - <<'PY'
from pathlib import Path
text = Path("/tmp/sts-day8-db-push.out").read_text(errors="replace") if Path("/tmp/sts-day8-db-push.out").exists() else ""
...
pass "Day 7 and Day 8 migrations are present"
...
echo
echo "DAY8_LOCAL_SUPABASE_VERIFY_PASSED"
