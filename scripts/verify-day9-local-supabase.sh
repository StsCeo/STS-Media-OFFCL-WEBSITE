#!/usr/bin/env bash
# Verify Day 9 client-portal migrations and RLS on the disposable local Supabase stack.
# Resets the isolated local database unless DAY9_DB_RESET=0. Does not touch production.
...
echo "Day 9 local Supabase verification"
...
if [[ "${DAY9_DB_RESET:-1}" == "1" ]]; then
  ...
  if ! npx --yes supabase db push --local >/tmp/sts-day9-db-push.out 2>&1; then
    if ! npx --yes supabase migration up --local >/tmp/sts-day9-db-push.out 2>&1; then
      python3 - <<'PY'
from pathlib import Path
text = Path("/tmp/sts-day9-db-push.out").read_text(errors="replace") if Path("/tmp/sts-day9-db-push.out").exists() else ""
...
EXPECTED=(
  ...
  "supabase/migrations/20260920200000_day9_client_portal.sql"
)
...
pass "Day 8 and Day 9 migrations are present"
...
run_sql supabase/tests/day8_isolation_runtime.sql DAY8_ISOLATION_RUNTIME_PASSED
run_sql supabase/tests/day9_isolation_runtime.sql DAY9_ISOLATION_RUNTIME_PASSED

echo
echo "DAY9_LOCAL_SUPABASE_VERIFY_PASSED"
