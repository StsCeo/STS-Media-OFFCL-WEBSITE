#!/usr/bin/env bash
# Verify Day 6 conversion migrations and RLS on the disposable local Supabase stack.
# Resets the isolated local database unless DAY6_DB_RESET=0. Does not touch production.
...
echo "Day 6 local Supabase verification"
...
if [[ "${DAY6_DB_RESET:-1}" == "1" ]]; then
  ...
  if ! npx --yes supabase db push --local >/tmp/sts-day6-db-push.out 2>&1; then
    if ! npx --yes supabase migration up --local >/tmp/sts-day6-db-push.out 2>&1; then
      python3 - <<'PY'
from pathlib import Path
text = Path("/tmp/sts-day6-db-push.out").read_text(errors="replace") if Path("/tmp/sts-day6-db-push.out").exists() else ""
...
EXPECTED=(
  "supabase/migrations/20260920160000_day5_estimates.sql"
  "supabase/migrations/20260920161000_day5_estimates_rpcs.sql"
  "supabase/migrations/20260920162000_day5_estimates_no_hard_delete.sql"
...
pass "Day 5 and Day 6 migrations are present"
...
echo
echo "DAY6_LOCAL_SUPABASE_VERIFY_PASSED"
