...
import { archiveProjectForm, saveProjectForm } from "@/app/actions";
  ...
  return (
    <div className="grid gap-3">
      <form action={saveProjectForm} className="grid gap-3 md:grid-cols-2">
        ...
        <select name="clientId" className={inputClass} defaultValue={project?.clientId || ""}>
        ...
        <input name="budget" type="number" step="0.01" min="0" className={inputClass} defaultValue={project?.budget ?? 0} placeholder="Budget (USD)" />
      ...
      {project ? (
        <form action={archiveProjectForm}>
          ...
          <Button type="submit" variant="secondary">Archive project</Button>
        </form>
      ) : null}
    </div>
  );
